...
After participants gave judgements on 60 sentences, there was a short demographic survey asking about their age, gender, first language(s) and the country or countries where they were raised. All of the response fields for those questions allowed for any kind of text answer to be typed. The duration of the whole experiment was 15-20 minutes. Appendix \ref{ap:exp-materials} contains the first page of the experiment with the consent form, a screenshot of an acceptability judgement trial and the information latter that participants could read before agreeing to participate in the study.
...
As \cite{skowronska_konstrukcje_2011} reports, the latter of the two additional nouns is problematic for Polish speakers. In a section dedicated specifically to the lexeme `chłopak' she reports that some Polish speakers are taught that the non-depreciative form of the noun is incorrect. Because of this, hybrid agreement is very commonly used with this particular noun. The author also looked through the National Corpus of Polish to see some examples of the noun with predicate (in (\ref{ex:chlopaki-pred})) and attributive agreement (in (\ref{ex:chlopaki-attr})). The numbers in parentheses correspond to the number of occurrences of a particular pair she found in the corpus.

\begin{exe}
  \ex\label{ex:chlopaki-pred}
  \begin{xlist}
    \ex 
    \gll chłopa-ki by-li/by-ły (11 / 9)\\
    boy\textsc{-pl.nom.depr} be\textsc{-pst.3pl.m1}/be\textsc{-pst.3pl.nm1}\\
    \glt ``boys were''

    \ex 
    \gll chłopa-ki zrobi-li/zrobi-ły (9 / 0)\\
    boy\textsc{-pl.nom.depr} do\textsc{-pst.3pl.m1}/do\textsc{-pst.3pl.nm1}\\
    \glt ``boys did''

    \ex 
    \gll chłopa-ki chci-eli/chci-ały (5 / 1)\\
    boy\textsc{-pl.nom.depr} want\textsc{-pst.3pl.m1}/want\textsc{-pst.3pl.nm1}\\
    \glt ``boys wanted''

    \ex 
    \gll chłopa-ki mówi-li/mówi-ły (7 / 1)\\
    boy\textsc{-pl.nom.depr} speak\textsc{-pst.3pl.m1}/speak\textsc{-pst.3pl.nm1}\\
    \glt ``boys spoke''
  \end{xlist}
\end{exe}

\begin{exe}
  \ex\label{ex:chlopaki-attr}
  \begin{xlist}
    \ex 
    \gll fajn-i/fajn-e chłopa-ki (1 / 70)\\
    cool\textsc{-m1.pl.nom}/cool\textsc{-nm1.pl.nom} boy\textsc{-pl.nom.depr} \\
    \glt ``cool boys''

    \ex 
    \gll mądr-zy/mądr-e chłopa-ki (0 / 4)\\
    smart\textsc{-m1.pl.nom}/smart\textsc{-nm1.pl.nom} boy\textsc{-pl.nom.depr} \\
    \glt ``smart boys''

    \ex 
    \gll ci/te chłopa-ki (0 / 33)\\
    \textsc{dem(m1.pl.nom)}/\textsc{dem(nm1.pl.nom)} boy\textsc{-pl.nom.depr} \\
    \glt ``those boys''
  \end{xlist}
\end{exe}

We can see that predicates with this noun are mostly seen with semantic agreement, while attributes occur almost exclusively with syntactic agreement. This is in line with Corbett's predictions and the current study can test whether this tendency extends to relative pronouns too.

All of the nouns chosen for this experiment can be found in Appendix \ref{ap:exp-items}. In (\ref{ex:depr-item}) there is an example of an experimental item with one of the nouns in depreciative form. 
...
\begin{exe}
  ...
  \begin{xlist}
    \ex f/m1, prenominal
    \gll Na nagrania do Ameryki Południowej \synt{poleci-ały}/\sem{poleci-eli} reżyser-y.\\
    ...
    \ex f/m1, postnominal
    \gll Reżysery \synt{poleci-ały}/\sem{poleci-eli} na nagrania do Ameryki Południowej.\\
    ...
    \gll Reżysery, \synt{któr-e}/\sem{któr-zy} lecą na nagrania do Ameryki Południowej, nie odpowiadają teraz na maile.\\
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex f/m1, prenominal
    \gll We wtorek \sem{przysz-ła}/\synt{przysz-edł} architekt-Ø.\\
    on Tuesday came\textsc{-pst.3sg.f}/came\textsc{-pst.3sg.m} architect\textsc{(m1)-sg.nom}\\
    \glt ``The architect came on Tuesday.''\\
    
    \ex f/m1, postnominal
    \gll Architekt-Ø \sem{przysz-ła}/\synt{przysz-edł} we wtorek.\\
    architect\textsc{(m1)-sg.nom} came\textsc{-pst.3sg.f}/came\textsc{-pst.3sg.m} on Tuesday\\
    \glt ``The architect came on Tuesday.''\\
    
    \ex f/m1, relative pronoun
    \gll Architekt-Ø, \sem{któr-a}/\synt{któr-y} przyjdzie we wtorek, należy do Rady Rodziców.\\
...
Both experiments also had 12 control items, half correct, to establish a threshold of acceptable and unacceptable sentences and test participants' sensitivity to gender agreement. The incorrect sentences contained an attraction error with the subject, the correct ones were the same but without the error \citep{bock1991broken}. These items have been adapted from the experiment conducted by \cite{an_closest_2022}. Half of the controls had human subjects and half had non-human, this was distributed evenly within the conditions. In the profession nouns experiment these items were preceded by a sentence of context, so that the experimental item would not be too easy to distinguish. Examples of a control items can be found in (\ref{ex:control-item-hum}) and (\ref{ex:control-item-nonhum}).

\begin{exe}
  \ex\label{ex:control-item-hum} Example of a control item with a human subject in both conditions and the context sentence.
  \begin{xlist}
    \ex context\\
...
\begin{exe}
  \ex\label{ex:control-item-nonhum} Example of a control item with a non-human subject in both conditions and the context sentence.
  \begin{xlist}
    \ex context\\
    Usłyszałam hałas w kuchni.\\
    \textsl{I heard noise in the kitchen.}

    \ex correct/incorrect
    \gll Pokrywka-Ø garn-ka spadł-a/*spad-ł na ziemię.\\
    lid\textsc{(f)-sg.nom} pot\textsc{(m3)-sg.gen} fall\textsc{-pst.3sg.f}/fall\textsc{-pst.3sg.m3} on ground\\
    \glt ``The pot's lid fell on the ground.''\\
  \end{xlist}
\end{exe}

To make sure the main focus of the study is not too obvious to participants, there were also 24 filler items adapted from an experiment with relative clauses and coordination testing \textsl{wh-} islands. As with control items there was a context sentence preceding the fillers in the profession noun experiment. An example can be found in (\ref{ex:filler-item}).
...
We analyzed the data from this experiment with the same mixed-effects ordinal regression model as in the previous section. There were the same fixed effects (agreement, order and target type) and the same random effects. In this model there were no significant effects, neither main effects nor interactions. The list of coefficients for this model is in Appendix \ref{ap:exp-coefs} (Table \ref{tab:coef-exp-depr}).

\vspace{-.5cm}
\section{Discussion}
The only significant effect found in the model with all nouns was that items with depreciative forms were rated significantly lower than items with profession nouns. According to one of the hypotheses, semantic agreement with split hybrids is less acceptable than with lexical hybrids, but here the lower ratings for depreciative forms were independent of agreement: both items with semantic and syntactic agreement were rated as close to unacceptable. There was no significant effect of interaction between agreement and type of noun.

Responses from two participants were filtered out because they rated all depreciative items as completely unacceptable. It seems that those participants thought the depreciative forms are unacceptable or incorrect and it is possible that this was the case to some extent with most of participants. Depreciative forms are a part of colloquial language and participants could feel that in the experimental settings those forms sound unnatural.

For the profession noun data, it should be noted that semantic agreement had similar ratings to syntactic agreement in the experiment, but not in the corpus study. This could mean that profession nouns in the corpus study had in fact more semantic agreement because of the appositions, that were used to identify potential hybrid agreement.

Based solely on descriptive statistics, in the results from Experiment 1 there seem to be some tendencies in the data in line with our hypotheses. On average semantic agreement is more acceptable with postnominal predicates than with prenominal predicates and target following the controller should make semantic agreement more acceptable, but the interaction between agreement and order was not significant. The items with relative pronouns have higher acceptability than items with predicates, and this difference was statistically significant but it was independent of agreement. Since the model showed no significant interaction between agreement and type of target, no conclusions can be drawn regarding acceptability of semantic agreement being higher for relative pronouns than for predicates, as would be concurrent with the full constraint of the Agreement Hierarchy.

As for depreciative forms, semantic agreement had higher average ratings than syntactic agreement regardless of the condition. Semantic agreement is expected to be less acceptable in general with split hybrids, yet both in the corpus study and in the experimental study it was surprisingly acceptable. What is most striking though, is that in the corpus study semantic agreement constituted only 20\% of all agreement with depreciative forms, but in the experimental study it is almost as acceptable as syntactic agreement. It is possible that in the experiment, when participants were faced with sentences with ``incorrect'' forms of nouns, they preferred the version of the sentence with agreement that would be used with the ``correct'' form of the noun. Even if there were some differences between acceptability in different conditions, none of them were statistically significant.
...
It would also be interesting to consider a different experimental design, for example a sentence recall task, where agreement chosen by participants when recalling the experimental items can show which agreement options they prefer. A forced choice task could also be used here, to see speakers' production preferences.
