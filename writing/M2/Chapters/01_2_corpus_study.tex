...
The corpus used for this study is the 300M subcorpus of the National Corpus of Polish \citep{przepiorkowski_narodowy_2012}, which contains texts from various genres and sources, including fiction, journals, letters, Internet, spoken and educational texts, most of which were created after the year 1990. The corpus was parsed using Trankit \citep{nguyen_trankit_2021} to obtain dependency trees conforming to the Universal Dependency standard (UD). Those dependency trees were searched for words, which could trigger hybrid agreement: profession nouns, socially gendered nouns and depreciative forms of \textsc{m1} nouns. The dependencies of those words then made it possible to search for attributive agreements (adjectives), predicate agreements (verbs agreeing with their subjects) and relative pronoun agreement. A tagged corpus could have been sufficient for attributive and predicate agreement, but using a parsed corpus gave a better chance of finding the correct relations and agreements within them and made it possible to include relative pronouns in the analysis. The subsections below describe how I chose the nouns for analysis.
...
\begin{table}[p]
  ...
  \begin{tblr}
    ...
    column{3} = {.1\linewidth},
    column{4} = {.5\linewidth},
    }
    Noun in Polish & Syntactic gender & Semantic gender & Translation or definition in English\\
    babochłop & \textsc{m2} & \textsc{f} & a woman that resembles a man with her physicality, looks and behavior\\
    babol & \textsc{m2} & \textsc{f} & a woman that looks or behaves in a way that causes aversion in the speaker\\
    babon & \textsc{m2} & \textsc{f} & a woman, about whom the speaker feels negatively\\
    babsko & \textsc{n} & \textsc{f} & a woman, about whom the speaker feels negatively\\
    babsztyl & \textsc{m2} & \textsc{f} & a woman, about whom the speaker feels negatively\\
    babus & \textsc{m2} & \textsc{f} & a woman, about whom the speaker feels negatively\\
    chłopaczysko & \textsc{n} & \textsc{m1} & a tall or big young man\\
    chłopobaba & \textsc{f} & \textsc{m1} & a man that resembles a woman with his physicality, looks and behavior (can also mean a woman that resembles a man)\\
    ciota & \textsc{f} & \textsc{m1} & a homosexual man (can also refer to a woman about whom the speaker feels negatively)\\
    dziewczątko & \textsc{n} & \textsc{f} & a girl, whom the speaker likes or to whom they feel compassion\\
    dziewczę & \textsc{n} & \textsc{f} & a young girl\\
    dziewuszysko & \textsc{n} & \textsc{f} & a girl who causes aversion or contempt in the speaker\\
    kobiecisko & \textsc{n} & \textsc{f} & expressively about a woman, augumentative, or to refer to a big woman\\
    kobieciątko & \textsc{n} & \textsc{f} & expressively about a woman, diminutive\\
    kobieton & \textsc{m2} & \textsc{f} & a woman that resembles a man with her physicality, looks and behavior\\
    pacholę & \textsc{n} & \textsc{m1} & a boy\\
    pannisko & \textsc{n} & \textsc{f} & expressively about a young woman, augumentative\\
    pasztet & \textsc{m3} & \textsc{f} & an ugly and unattractive woman\\
    wamp & \textsc{m2} & \textsc{f} & a confident, misterious, provocative and attractive woman who seduces men\\
  \end{tblr}
...
\begin{exe}
  ...
  \gll Chłop-y ze wsi pomaga-li tutaj.\\
...
\begin{figure}[h]
  ...
  \begin{dependency}
    \begin{deptext}
      Starost-a \& konińsk-i \& swój \& puchar \& przyzna-ła \& firmie \& Roof \& Reed.\\
...
The 214 917 initial observations were filtered to remove those, which were mistakenly included. There were 263 agreement observations with the gendered \textsc {m3} noun \textsl{pasztet}, which as a gendered noun means a girl or a woman whom the speaker finds unattractive, but it has another, much more common meaning: pâté, a kind of forcemeat. Most of the occurrences found in the corpus were in the second meaning, not useful for the study, therefore I removed this noun completely from the study.
...
To figure out whether an occurrence of a profession noun referred to a man or a woman, I added information about appositions attached to the nouns. I removed occurrences with plural and non-\textsc{f} appositions, because the plural appositions were mostly parsing mistakes and the non-\textsc{f} appositions show us that the referent of the noun is not a woman and there cannot be any hybrid agreement, as can be seen in ((\ref{ex:prof-appos-m1})). Since it is possible that in some cases we agree the target with the gender of the apposition and not the gender of the controller, I marked the ordering of the target, controller and apposition for each agreement to be able to look into the influence of the apposition. In (\ref{ex:prof-occurrence}) for example, the order was CTA. Only this selection of occurrences with profession nouns was saved for further analysis.
...
\begin{exe}
  ...
  \glt ``Our house was designed by a little known architect.''\\
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \gll \textbf{Tutej-sze}/tutej-si wykształciuch-y nie są polskimi inteligentami.\\
    \textbf{local\textsc{-nm1.pl.nom}}/local\textsc{-m1.pl.nom} educated.person\textsc{-pl.nom.depr} \textsc{neg} be\textsc{(prs.3pl)} Polish intellectuals\\
    \glt ``The local educated people are not the Polish intellectuals.''\\
    NCP 300M, sentence ID: 12590744\\

    \ex\label{ex:depr-pred}
    \gll Mieszczuch-y \textbf{wol-ały-by}/wol-eli-by, żeby chichotał i mrugał porozumiewawczo.\\
    city.dwellers\textsc{-pl.nom.depr} \textbf{prefer\textsc{-3pl.nm1-cond}}/prefer\textsc{-3pl.m1-cond} that giggle and wink knowingly\\
    \glt ``The city dwellers would prefer him to giggle and wink knowingly.''\\
    NCP 300M, sentence ID: 3883941\\

    \ex\label{ex:depr-relpron}
    \gll Mnie najbardziej zbijają z nóg modnisi-e, \textbf{któ-re}/któ-rzy uważają się za orginalne.\\
    me most knock off legs dandy\textsc{-pl.acc.depr} \textbf{which\textsc{-nm1.pl.nom}}/\textsc{-m1.pl.nom} consider \textsc{refl} as original\\
    \glt ``Dandys, who consider themselves original, surprise me the most.''\\
    NCP 300M, sentence ID: 6767871\\
  \end{xlist}
\end{exe}

Binomial logistic regression using only the agreement occurrences with depreciative forms with the type of agreement target, distance and target placement as predictors has shown that only target placement had a significant effect on the outcome -- target following the controller increased the likelihood of semantic agreement (\textsl{p}<0.001).
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \gll Zachichota-ł jak \textbf{stuprocentow-a}/stuprocentow-y ciot-a.\\
    giggle\textsc{-pst.3sg.m1} like \textbf{hundred.percent\textsc{-f.sg.nom}}/hundred.percent\textsc{-m1.sg.nom} faggot\textsc{(f)-sg.nom}\\
    \glt ``He giggled like a 100\% faggot.''\\
    NCP 300M, sentence ID: 18628964\\

    \ex\label{ex:gend-pred}
    \gll Kobiecisk-o \textbf{przytulił-o}/przytulił-a się histerycznie do mnie.\\
    woman\textsc{(n)-sg.nom} \textbf{hug\textsc{-pst.3sg.n}}/hug\textsc{-pst.3sg.f} \textsc{refl} histerically to me\\
    \glt ``The woman hugged me histerically.''\\
    NCP 300M, sentence ID: 16919204\\

    \ex\label{ex:gend-relpron}
    \gll Pamiętasz historię dziewczę-cia, \textbf{któr-e}/któr-a uciekło z balu?\\
    remember story girl\textsc{(n)-sg.gen} \textbf{which\textsc{-n.sg.nom}}/which\textsc{-f.sg.nom} ran.away from dance\\
    \glt ``Do you remember the story of the girl, who ran away from the dance?''\\
    NCP 300M, sentence ID: 18852664\\
  \end{xlist}
\end{exe}

Binomial logistic regression using only the agreement occurrences with socially gendered nouns with the type of agreement target, distance and target placement as predictors has shown that distance was the only significant predictor of semantic agreement: larger distance increased the likelihood of semantic agreement with gendered nouns (\textsl{p}<0.05).
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \gll Większość złożyli w końcówce kadencji obecne-go/\textbf{obecne-j} burmistrz-Ø Stefanii Sulińskiej.\\
    majority submitted in ending term current\textsc{-m1.sg.gen}/\textbf{current\textsc{-f.sg.gen}} mayor\textsc{(m1)-sg.nom} Stefania Sulińska \\
    ...
    \gll One są gotowe do adaptacji do roku 2008 – \textbf{zapewniał-Ø}/zapewniał-a minist-er edukacji Krystyna Łybacka.\\
    they are ready for adaptation to year 2008 - \textbf{assured\textsc{-pst.3sg.m1}}/assured\textsc{-pst.3sg.f} minister\textsc{(m1)-sg.nom} education Krystyna Łybacka\\
    ...
    \gll Felicity Huffman gra panią naukow-iec, któr-y/\textbf{któr-a} w arktycznej stacji zostaje zarażona pasożytem z kosmosu.\\
    Felicity Huffman plays lady scientist\textsc{(m1)-sg.nom} which\textsc{-m1.sg.nom}/\textbf{which\textsc{-f.sg.nom}} in arctic station becomes infected parasite from space\\
...
Considering the effect of apposition placement on the choice of agreement, we can look at how the proportions of semantic agreement change if we assume that we should exclude occurrences with apposition first, because the speakers agree the gender of the target with the gender of apposition and not the semantic gender of the controller. Figure \ref{fig:NOAPP-semantic-in-targets} is identical to Figure \ref{fig:semantic-in-targets}, only with occurrences of profession nouns with appositions first excluded.
