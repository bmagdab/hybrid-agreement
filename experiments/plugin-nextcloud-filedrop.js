var jsPsychNextcloudFiledropPlugin = (function (jspsych) {
  ...
  class NextcloudFiledropPlugin {
    ...
    trial(display_element, trial) {
			// MB: .csv() instead of .json()
      let data = this.jsPsych.data.get().csv();

      // Determine filename
			// let filename =
        trial["filename"] == null ? new Date().toJSON() + ".zip" : trial["filename"](this.jsPsych);
			// MB: here again json and zip changed to csv
      let filename =
        trial["filename"] == null ? new Date().toISOString() + ".csv" : trial["filename"] + ".csv";
      ...
      this.jsPsych.getDisplayElement().appendChild(progress_bar_container);
			...
			// MB: here I removed some more zipping code and defined the data blob differently
          ...
          xhr.upload.addEventListener("progress", (event) => {
            progress_bar_text.innerHTML = "Przesyłanie danych, proszę czekać.";
          ...
          //const url = trial["url"] + "/public.php/webdav/" + filename;
					const url =
  trial["url"] + "/public.php/dav/files/" + trial["folder"] + "/" + filename; // Bug fix
